...
def master():
    ...
    conn.close()
    return render_template('master.html', items=items, categories=categories, last_category=last_category)
...
@app.route('/trip/<int:trip_id>/pack/<int:item_id>/<user>')
...
@app.route('/trip/<int:trip_id>/unpack/<int:item_id>')
...
@app.route('/trip/<int:id>/add_items', methods=['GET', 'POST'])
def add_trip_items(id):
    conn = get_db()
    if request.method == 'POST':
        selected = request.form.getlist('items')
        for item_id in selected:
            # Only add if not already on the trip
            exists = conn.execute(
                'SELECT id FROM trip_items WHERE trip_id = ? AND master_item_id = ?',
                (id, item_id)
            ).fetchone()
            if not exists:
                conn.execute(
                    'INSERT INTO trip_items (trip_id, master_item_id) VALUES (?, ?)',
                    (id, item_id)
                )
        conn.commit()
        conn.close()
        return redirect(url_for('trip', id=id))
    # Get items NOT already on this trip
    trip = conn.execute('SELECT * FROM trips WHERE id = ?', (id,)).fetchone()
